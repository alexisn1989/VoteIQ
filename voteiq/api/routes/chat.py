...
def _bills_system_prompt(
    ...
    return (
        ...
        f"- Sector ranking language: write '[Sector] ranks Nth among classified industry sectors, excluding $X in Retired/Individual and Unknown donations' — never write 'ranks Nth out of N sectors' or similar. Always call out excluded buckets explicitly when they exceed any ranked sector.\n\n"
        ...
        f"- Policy-area vote label rule: when grouping votes by national security or military topics, the section label MUST be 'National Security / Defense-adjacent roll calls (VoteIQ classification)' — never 'Defense-Related Votes' or similar. Add one line below every such section: 'Classification is based on bill subject tagging, not legislative intent.'\n\n"
        ...
        f"ALWAYS begin any congressional vote table or vote list with the caption: "
        f"'Chamber: U.S. House of Representatives (119th Congress roll calls)'. Never mix House and Senate votes in the same table.\n"
        f"- Campaign finance metadata block: At the end of EVERY response that uses any campaign finance data block "
        f"(fec_individual_contributions, fec_independent_expenditures, va_cf_schedule_a), append this exact block:\n"
        f"  **Data Coverage**\n"
        f"  - FEC contributions: partial employer-tag mapping\n"
        f"  - Outside spending: full independent expenditure dataset\n"
        f"  - Roll-call votes: House Clerk, listed bills only\n\n"
        f"  **Confidence**\n"
        f"  - Contributions: MEDIUM (classification-dependent)\n"
        f"  - Outside spending: HIGH\n"
        f"  - Votes: HIGH\n"
