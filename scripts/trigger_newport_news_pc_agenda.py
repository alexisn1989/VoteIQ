#!/usr/bin/env python3
"""
trigger_newport_news_pc_agenda.py

Called by the Render cron job "voteiq-newport-news-pc-agenda".
POSTs to /api/admin/ingest-newport-news-pc-agenda on the running web service
so the scraper runs inside the container that has access to
/var/data/polls.db.
...
url = f"{SERVICE_URL}/api/admin/ingest-newport-news-pc-agenda"
print(f"POST {url}")
