#!/usr/bin/env python3
"""
trigger_agenda_geocoding.py

Called by the Render cron job "voteiq-agenda-geocoding".
POSTs to /api/admin/ingest-agenda-geocoding on the running web service so the
geocoder runs inside the container that has access to /var/data/polls.db.
...
url = f"{SERVICE_URL}/api/admin/ingest-agenda-geocoding"
print(f"POST {url}")
