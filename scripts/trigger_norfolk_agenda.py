#!/usr/bin/env python3
"""
trigger_norfolk_agenda.py

Called by the Render cron job "voteiq-norfolk-agenda".
POSTs to /api/admin/ingest-norfolk-agenda on the running web service so the
scraper runs inside the container that has access to /var/data/polls.db.
...
DAYS        = int(os.environ.get("AGENDA_DAYS", "90"))

if not SERVICE_URL:
    print("ERROR: VOTEIQ_SERVICE_URL is not set.", file=sys.stderr)
    sys.exit(1)

url = f"{SERVICE_URL}/api/admin/ingest-norfolk-agenda?days={DAYS}"
print(f"POST {url}")
