#!/usr/bin/env python3
"""
trigger_congress_ingest.py

Called by the Render cron job "voteiq-congress-weekly".
POSTs to /api/admin/ingest-congress on the running web service so the
...
Optional:
  HOUSE_LIMIT         — default 500
  SENATE_LIMIT        — default 300 (passed via query param if supported)
"""
...
HOUSE_LIMIT  = int(os.environ.get("HOUSE_LIMIT",  "500"))

if not SERVICE_URL:
    print("ERROR: VOTEIQ_SERVICE_URL is not set.", file=sys.stderr)
    sys.exit(1)

url = f"{SERVICE_URL}/api/admin/ingest-congress?house_limit={HOUSE_LIMIT}"
print(f"POST {url}")
...
try:
    with urllib.request.urlopen(req, timeout=660) as resp:
        ...
        print(f"ok={data.get('ok')}  scripts={list(data.get('scripts', {}).keys())}")
