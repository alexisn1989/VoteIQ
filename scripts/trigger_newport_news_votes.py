#!/usr/bin/env python3
"""
trigger_newport_news_votes.py

Called by the Render cron job "voteiq-newport-news-votes".
POSTs to /api/admin/ingest-newport-news-votes on the running web service so
the scraper runs inside the container that has access to /var/data/polls.db.
...
url = f"{SERVICE_URL}/api/admin/ingest-newport-news-votes"
print(f"POST {url}")
...
try:
    with urllib.request.urlopen(req, timeout=440) as resp:
        body = resp.read().decode("utf-8", errors="replace")
        data = json.loads(body)
        print(f"Status: {resp.status}")
        for script, result in data.get("scripts", {}).items():
            ok  = result.get("ok", "?")
            out = (result.get("stdout") or "")[-800:]
