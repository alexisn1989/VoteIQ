...
from voteiq.services.data_meta import ensure_schema as _ensure_data_meta, get_all as _get_data_meta, ensure_performance_indexes as _ensure_performance_indexes, cleanup_stale_bill_descriptions as _cleanup_stale_bill_descriptions
...
# Reply layer extracted to voteiq/llm/reply.py (main.py decomposition, phase 1).
# Re-imported here so existing `import main as _m` callers keep working; new
# code should import from voteiq.llm.reply directly.
from voteiq.llm.reply import (  # noqa: F401, E402
    _CLAUDE_HAIKU_MODEL,
...
_DATA_CATALOG: list[dict] = [
    ...
    # ── Local Government (Hampton Roads city councils) ─────────────────────────
    {"table": "norfolk_council_member_votes",      "label": "Norfolk Council Votes (per-member)",      "category": "Local Government", "source": "City of Norfolk IQM2 minutes",            "source_url": "https://norfolkcityva.iqm2.com/Citizens/calendar.aspx"},
    {"table": "vb_council_member_votes",           "label": "Virginia Beach Council Votes (per-member)", "category": "Local Government", "source": "City of Virginia Beach Accela IQM2 API", "source_url": "https://virginiabeachva.iqm2.com"},
    {"table": "chesapeake_council_member_votes",   "label": "Chesapeake Council Votes (per-member)",   "category": "Local Government", "source": "City of Chesapeake Granicus archive",     "source_url": "https://chesapeake.granicus.com"},
    {"table": "portsmouth_council_member_votes",   "label": "Portsmouth Council Votes (per-member)",   "category": "Local Government", "source": "City of Portsmouth minutes archive",      "source_url": "https://www2.portsmouthva.gov/weblink7CCMinutes/"},
    {"table": "newport_news_council_member_votes", "label": "Newport News Council Votes (per-member)", "category": "Local Government", "source": "City of Newport News CivicWeb voting records", "source_url": "https://nngov.civicweb.net/Portal/VotingRecords.aspx"},
    {"table": "hampton_council_member_votes",      "label": "Hampton Council Votes (per-member)",      "category": "Local Government", "source": "City of Hampton Legistar minutes",        "source_url": "https://hampton.legistar.com"},
    {"table": "suffolk_council_member_votes",      "label": "Suffolk Council Votes (per-member)",      "category": "Local Government", "source": "City of Suffolk Agenda Center",           "source_url": "https://www.suffolkva.us/AgendaCenter"},
    {"table": "sbe_local_contributions",           "label": "Local Campaign Contributions (7 cities)", "category": "Local Government", "source": "Virginia Department of Elections bulk CSV", "source_url": "https://apps.elections.virginia.gov/SBE_CSV/CF/"},
    # ── Analysis Layers ────────────────────────────────────────────────────────
...
_safe_include("voteiq.api.routes.council_member")
_safe_include("voteiq.api.routes.agenda_alerts",       prefix="/api")
...
# Startup ingestion threads (polls/fec-2026/federal-alignment/party-breakdown/
# schedule-e/congress/committees) moved to Render crons calling the admin
# endpoints in voteiq/api/routes/admin.py — same fix as the FEC-PAC startup
# thread above: each one re-ran (up to 30 min) inside the web container on
# every deploy while data was stale, on both gunicorn workers, repeatedly
# timing out and pressuring the 512MB instance. See render.yaml for the
# cron schedule and scripts/trigger_*.py for the trigger scripts.
...
async def _startup_ingest() -> None:
    ...
    try:
        _cleanup_stale_bill_descriptions()  # purge pre-dedup 2026 cached descriptions
    except Exception as _exc:
...
# _gemini_reply / _GEMINI_MODEL moved to voteiq/llm/reply.py (re-imported above)
...
# Claude reply layer (client, _claude_reply, _friendly_claude_error,
# _graceful_truncate, model constants) moved to voteiq/llm/reply.py and
# re-imported near the top of this file for `import main as _m` callers.
...
# Reply cache moved to voteiq/services/reply_cache.py (decomposition phase 2).
from voteiq.services.reply_cache import (  # noqa: F401, E402
    _CACHE_TTL_SECONDS,
    _cache_key,
    _get_cached_reply,
    _init_query_cache,
...
# District/member static data moved to voteiq/config/districts.py
# (decomposition phase 2). Re-imported so `import main as _m` callers and
# main-internal users (flip maps at _build_*) keep working unchanged.
from voteiq.config.districts import (  # noqa: F401, E402
    DISTRICT_CONTEXT,
    HOD_CONTEXT,
    HOD_2021_PARTY,
    HOD_2023_PARTY,
...
# _session_year moved to voteiq/utils.py (decomposition phase 2)
from voteiq.utils import _SESSION_YEAR_RE, _session_year  # noqa: F401, E402
